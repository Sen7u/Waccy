...
当前可用：**文本历史 MVP** — 监听剪贴板、搜索、置顶、删除、墨青弹出层动效；托盘可暂停捕获 / 切换「选中后默认粘贴」；设置窗可改历史上限、热键、忽略应用（JSON 落盘到 `%LocalAppData%/Inkboard/settings.json`）。Linux 用窗口内 `Ctrl+Shift+V` / 托盘唤起，Windows 注册全局热键并记录来源进程名。
...
| `Inkboard.Infrastructure.Persistence` | 存储（历史内存；设置 JSON） |
